...
export default {
    ...
    methods: {
        ...
        async share() {
            ...
            const title = this.seller?.store_name || 'QuêTôi';
            try {
                if (navigator.share) {
        ...
        <div class="hero">
            ...
            <div class="hero-actions">
                ...
                <button class="hero-btn" @click="toggleFavorite" :aria-pressed="isFavorite" aria-label="Save to favorites">
                    <Heart :size="18" :fill="isFavorite ? '#1D6B4A' : 'none'" :color="isFavorite ? '#1D6B4A' : '#fff'" />
                </button>
            </div>
        </div>
                ...
                <div class="meta-row">
                    <div class="status-line">
                        <span v-if="hasRating" class="rating">
                            <Star :size="13" fill="#1D6B4A" color="#1D6B4A" /> {{ seller.rating }}
                        </span>
                    ...
                    <button v-if="!isOwnStore" class="chat-btn" :disabled="isOpeningChat" @click="chatWithSeller">
                        <q-spinner-dots v-if="isOpeningChat" color="white" size="1.4em" />
                        <template v-else>
...
.avatar-ring {
    ...
    box-shadow: 0 0 0 2px rgba(29, 107, 74, 0.4);
    line-height: 0;
...
.chat-btn {
    ...
    font-weight: 700;
    cursor: pointer;
    box-shadow: 0 6px 16px -6px rgba(29, 107, 74, 0.5);
    transition: transform 0.1s ease, box-shadow 0.15s ease, opacity 0.15s ease;

    &:active {
        transform: translateY(1px);
        box-shadow: 0 3px 10px -6px rgba(29, 107, 74, 0.5);
    }
