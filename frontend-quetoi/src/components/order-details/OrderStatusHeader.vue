...
export default {
    ...
    computed: {
        ...
        statusIconColor() {
            if (this.isPaymentPending) return '#C9A55A'
            const map = {
                pending: '#C9A55A',
                confirmed: '#2A8A5E',
                ready: '#2A8A5E',
                ...
                cancelled: '#C0392B',
                delivery_failed: '#C9A55A',
            }
            return map[this.order?.status] || '#1A2A20'
        },
        refundStatus() {
...
.refund-pending   { background: rgba(201, 165, 90, 0.15); color: var(--color-warning); }
.refund-processed { background: rgba(42, 138, 94, 0.12);  color: var(--color-success); }
.refund-failed    { background: rgba(192, 57, 43, 0.12);   color: var(--color-error); }
