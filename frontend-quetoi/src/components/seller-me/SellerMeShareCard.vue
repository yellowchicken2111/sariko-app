    ...
    methods: {
        async _renderQR(url) {
            ...
            await QRCode.toCanvas(canvas, url, {
                width: 120,
                margin: 1,
                color: { dark: '#1A2A20', light: '#ffffff' },
            })
        },
        ...
        downloadQR() {
            ...
            link.download = `quetoi-qr-${this.sellerInfo?.slug || 'store'}.png`
...
.btn-primary {
    background: var(--color-primary, #1D6B4A);
    color: #F5F0E8;
    border: none;
}
...
.btn-secondary {
    ...
    color: var(--color-primary, #1D6B4A);
    border: 1px solid var(--color-primary, #1D6B4A);
}
